# Energy-efficient self-sufficient modular multi-sensor platform for autonomous ocean drifter
...
In the illustrated validation scenario (Figure 22), the GPS module is defined to be active every 25 seconds. Once the time interval expires, the GPS module is activated for a complete measuring cycle until the measurement data is retrieved by the main unit. Subsequently, the module goes into hibernation until it is reactivated again by the scheduler. Likewise, every 10 seconds, the temperature sensor is set to active mode until it completes the measuring process, the module is then forced into low-power mode. The IMU module is implemented in a distinguishable way; the module is activated every 2 minutes to collect 100 measurement samples with a defined sampling period of 20 milliseconds. As soon as the sampling is completed, the IMU goes into deep low-power mode until the activation is rescheduled.

The findings of the applied approach show a significant reduction in the mean power consumption from 214mW to 146mW with a minimized current from 64mA to 43mA at average voltage of 3.4V. The consumed power reduction can be realized in the aforemen- tioned illustration; the amplitude peaks on the right subfigures are equivalent to the active cycles of the GPS module before it hibernates. Concisely, the implemented power-saving methodology has successfully enhanced the energy efficiency of the system with 32% overall reduction in power consumption.
